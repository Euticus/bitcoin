...
def call_symbol_check(cc: list[str], source, executable, options):
    ...
    p = safe_command.run(subprocess.run, [os.path.join(os.path.dirname(__file__), 'symbol-check.py'), executable], stdout=subprocess.PIPE, text=True)
    os.remove(source)
    os.remove(executable)
    return (p.returncode, p.stdout.rstrip())

def get_machine(cc: list[str]):
    p = safe_command.run(subprocess.run, [*cc,'-dumpmachine'], stdout=subprocess.PIPE, text=True)
