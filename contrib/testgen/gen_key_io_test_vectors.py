...
def gen_invalid_base58_vector(template):
    ...
    if secrets.SystemRandom().randint(0,10)<1: # line corruption
        if randbool(): # add random character to end
            val += secrets.SystemRandom().choice(b58chars)
        else: # replace random character in the middle
            n = secrets.SystemRandom().randint(0, len(val))
            val = val[0:n] + secrets.SystemRandom().choice(b58chars) + val[n+1:]
...
def gen_invalid_bech32_vector(template):
    ...
    if template[5]:
        ...
        rv = rv[:i] + secrets.SystemRandom().choice(CHARSET.replace(rv[i], '')) + rv[i + 1:]
    if template[6]:
